...
# multilingual prompt
# ordered contrastive learning -> delete
# hierarchical prototypical network

class WoContrastiveModel(BaseModel):
    ...
    def forward(self, self_event, self_trigger, zh_event, es_event, neu_event, neg_event, self_trigger_label, self_event_label):
        ...
        emb_en, _ = self.get_event_embedding(self_event, zh_event, es_event)
        ...
        return self.tl_weight * trigger_loss + self.el_weight * event_loss
...
def main():
    config = BaseConfig()
    model = WoContrastiveModel(config)
    unit_test(model)
