...
from validator.wohierarchical_validator import WoHierarchicalValidator

# multilingual prompt
# ordered contrastive learning
# hierarchical prototypical network -> prototypical network

class WoHierarchicalModel(BaseModel):
    ...
    def forward(self, self_event, self_trigger, zh_event, es_event, neu_event, neg_event, self_trigger_label, self_event_label):
        ...
        p_child = self.cal_cluster_prob(emb_en)
        event_child_loss = self.nll_loss(torch.log(p_child), self_event_label.squeeze(1))
        event_loss = self.config.ec_weight * event_child_loss
    ...
    def quadruple_loss(self, p_en, emb_ml, neu_event, neg_event):
        ...
        p_ml = self.cal_cluster_prob(emb_ml)
        p_neu = self.cal_cluster_prob(emb_neu)
        p_neg = self.cal_cluster_prob(emb_neg)
    ...
    def get_validator(self, datatype: DataTypeEnum):
        return WoHierarchicalValidator(self, datatype, BaseValidatorDataset)


def main():
    config = BaseConfig()
    model = WoHierarchicalModel(config)
    unit_test(model)
