...
# multilingual prompt -> english prompt
# ordered contrastive learning -> triplet
# hierarchical prototypical network

class WoMultilingualModel(BaseModel):
    ...
    def forward(self, self_event, self_trigger, neu_event, neg_event, self_trigger_label, self_event_label):
        ...
        contrastive_loss = self.triplet_loss(p_child, neu_event, neg_event)
    ...
    def triplet_loss(self, p_en, neu_event, neg_event):
        ...
        return dist_neu + loss_neg

    def create_dataset(self, data_type: DataTypeEnum) -> Dataset:
        return BaseDataset(self.config.data_set, data_type, self.config.plm, self.config.k)
...
def main():
    config = BaseConfig()
    model = WoMultilingualModel(config)
    unit_test(model)
