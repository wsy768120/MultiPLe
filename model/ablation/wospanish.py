...
# multilingual prompt -> ez prompt
# ordered contrastive learning -> ml to ez
# hierarchical prototypical network

class WoSpanishModel(BaseModel):
    ...
    def forward(self, self_event, self_trigger, zh_event, es_event, neu_event, neg_event, self_trigger_label, self_event_label):
        ...
        emb_en, emb_ez = self.get_event_embedding(self_event, zh_event)
        ...
        contrastive_loss = self.quadruple_loss(p_child, emb_ez, neu_event, neg_event)
    ...
    def get_event_embedding(self, self_event, zh_event):
        emb_en = self.predict_event(self_event)
        emb_zh = self.predict_event(zh_event)
        # 拼接多语种事件表征，得到联合事件表征
        emb_ez = torch.divide(torch.add(emb_en, emb_zh), 2)
        return emb_en, emb_ez
    ...
    def quadruple_loss(self, p_en, emb_ez, neu_event, neg_event):
        emb_neu = self.predict_event(neu_event)
        emb_neg = self.predict_event(neg_event)

        _, p_ez = self.cal_cluster_prob(emb_ez)
        ...
        dist_ez = dist_func(p_en, p_ez)

        dist_neu = dist_func(p_en, p_neu)
        diff_neu = self.contrastive_loss_margin - (dist_neu - dist_ez)
        ...
        return dist_ez + loss_neu + loss_neg
