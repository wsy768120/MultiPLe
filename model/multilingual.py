...
class MultilingualModel(BaseModel):
    def __init__(self, config: BaseConfig):
        ...
        self.config = config
        self.trigger_mask_idx = 1  # trigger [mask] index
        self.event_mask_idx = 4  # event [mask] index xlm:4 bm:5
        self.mention_offset = 8  # event.mention start_index xlm:8 bm:9
    ...
    def forward(self, self_event, self_trigger, zh_event, es_event, neu_event, neg_event, self_trigger_label, self_event_label):
        ...
        emb_en, emb_ml = self.get_event_embedding(self_event, zh_event, es_event)
        ...
        contrastive_loss = self.quadruple_loss(p_child, emb_ml, neu_event, neg_event)
    ...
    def predict_trigger(self, event_encoding: Dict[str, torch.Tensor]):
        ...
        prediction_scores = self.plm_cls(xlm_out[0])  # xlm_out[0](4,100,1024) to prediction_scores(4,100,250002)
        logit = prediction_scores[:, self.trigger_mask_idx, :]  # 第0维取所有:，第1维取1,代表trigger的<MASK>，第2维取所有:. logit: Tensor(4,250002)
        mention_ids = self.mention_loc(event_encoding["input_ids"])  # tensor(4,100)
    ...
    def mention_loc(self, embedding: torch.Tensor):
        mention_ids_list = []
        for i in range(embedding.shape[0]):  # 4
    ...
    def get_event_embedding(self, self_event, zh_event, es_event):
        emb_en = self.predict_event(self_event)
        emb_zh = self.predict_event(zh_event)
        emb_es = self.predict_event(es_event)
        # 拼接多语种事件表征，得到联合事件表征
        emb_ee = torch.divide(torch.add(emb_en, emb_es), 2)
        emb_ml = torch.divide(torch.add(torch.multiply(emb_ee, 2), emb_zh), 3)
        return emb_en, emb_ml
    ...
    def predict_event(self, event_encoding: Dict[str, torch.Tensor]):
        ...
        mask_vector_list = []
        for i in range(xlm_out.last_hidden_state.shape[0]):  # xlm_out.last_hidden_state (4,100,1024)
    ...
    def quadruple_loss(self, p_en, emb_ml, neu_event, neg_event):
        emb_neu = self.predict_event(neu_event)
        emb_neg = self.predict_event(neg_event)

        _, p_ml = self.cal_cluster_prob(emb_ml)
        ...
        dist_ml = dist_func(p_en, p_ml)

        dist_neu = dist_func(p_en, p_neu)
        diff_neu = self.contrastive_loss_margin - (dist_neu - dist_ml)
        ...
        return dist_ml + loss_neu + loss_neg
...
def main():
    config = BaseConfig()
    model = MultilingualModel(config)
    unit_test(model)
