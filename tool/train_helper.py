...
class TrainHelper:
    ...
    def train_model(self):
        ...
        plm_params = []
        if self.model.plm_model:
            plm_params += list(self.model.plm_model.parameters())
        if self.model.plm_cls:
            plm_params += list(self.model.plm_cls.parameters())

        # both plm and plm_cls contains word embedding weight, so distinct here.
        plm_params_set = set(plm_params)
        plm_params = list(plm_params_set)
        other_params = [p for p in self.model.parameters() if p not in plm_params_set]
        opt = transformers.AdamW(
            [
                {"params": plm_params, "lr": self.config.plm_learning_rate},
        ...
        while self.model.current_training_step < self.total_train_steps:
            ...
            opt.zero_grad()
            batch = next(self.repeat_train_dataloader) # event_encoding
            self.model.move_batch_to_device(batch)
            loss = self.training_model(**batch).mean() # enter model!!
            loss.backward()
            opt.step()
    ...
    def prepare_training(self):
        ...
        # early stop only after one epoch (all train_dataset trained).
